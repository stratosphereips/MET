...
def getr_args():
    ...
    parser.add_argument(
        "--simplenet", action="store_true", help="Use SimpleNet instead of Resnet34"
    )
    parser.add_argument(
        "--resolution", default=224, type=int, help="Resolution of samples"
    )
    parser.add_argument(
        "--num_classes",
        default=5,
...
if __name__ == "__main__":
    ...
    transform = T.Compose(
        [T.Grayscale(num_output_channels=3), T.Resize((args.resolution,)), T.ToTensor()]
    )
    train_set = OIModular(
        args.oi_dir, args.num_classes, download=True, transform=transform
    )
    test_set = OIModular(
        args.oi_dir, args.num_classes, train=False, download=True, transform=transform
    )

    train_set, val_set = split_dataset(train_set, 0.2)

    if args.simplenet:
        test_model = SimpleNet((3, args.resolution, args.resolution), args.num_classes)
    else:
