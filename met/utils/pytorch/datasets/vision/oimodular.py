...
class OIModular(Dataset):
    ...
    def __init__(
        ...
        if num_classes not in [5, 17, 51]:
            raise ValueError(
                f"ModularOpenImages dataset has following "
        ...
        self._classes_to_idx = {
            name: idx for name, idx in zip(class_names, range(len(class_names)))
        }
        ...
        print(
            f"Loaded {self.__class__.__name__} ({self._split}) with "
    ...
    def _class_label_codes(self) -> Dict:
        ...
        df_classes = pd.read_csv(descriptions_csv_file_path, header=0)
            ...
            for _, subsubclasses in subclasses.items():
                for subsubclass in subsubclasses:
                    ...
                    if subsubclass == "Lavender":
                        labels_to_codes[subsubclass.lower()] = df_classes.loc[
                            df_classes["DisplayName"] == "Lavender (Plant)"
                        ].values[0][0]
                    else:
                        labels_to_codes[subsubclass.lower()] = df_classes.loc[
                            df_classes["DisplayName"] == subsubclass
                        ].values[0][0]

        # return the labels to OpenImages codes dictionary
        return labels_to_codes

    def _download_one_image(self, bucket, img_id: str, download_dir: Path):
        try:
    ...
    def _download_subsubclass_images(
        self, subsubclass_img_ids: np.array, subsubclass_dir: Path
    ):
        ...
        progress_bar = tqdm(
            total=num_samples,
            desc=f"Downloading images: Subsubclass: " f"{subsubclass_dir.stem}",
        )
        downloaded_images = 0
        dfs_imgs = []
        with futures.ThreadPoolExecutor(max_workers=10) as executor:
            for img_id in subsubclass_img_ids:
                future = executor.submit(
                    self._download_one_image, bucket, img_id, subsubclass_dir
                )
    ...
    def _download_images(self):
        ...
        for superclass, subclasses in self._openimages_classes.items():
            for subclass, subsubclasses in subclasses.items():
                for subsubclass in subsubclasses:
                    subsubclass_dir = self._root.joinpath(
                        superclass, subclass, subsubclass
                    )
                    subsubclass_code = self._labels_to_codes[subsubclass.lower()]

                    subsubclass_h_imgs = (
                        df_hv_labels["LabelName"] == subsubclass_code
                    ) & (df_hv_labels["Confidence"] == 1)
                    subsubclass_img_ids = (
                        df_hv_labels.loc[subsubclass_h_imgs].loc[:, "ImageID"].values
                    )

                    dfs_imgs.append(
                        self._download_subsubclass_images(
                            subsubclass_img_ids, subsubclass_dir
                        )
                    )

        df_all_imgs = pd.concat(dfs_imgs)
        df_all_imgs.to_csv(self._root.joinpath("img_ids_and_classes.csv"), index=False)

        return

    def _get_split_idxs(self) -> Dict[str, List[int]]:
        partition_to_imgs = {"train": [], "test": []}
        ...
        for idx, row in self._df_img_ids_classes.iterrows():
            filepath = str(
                self._root.joinpath(
                    row.Superclass, row.Subclass, row.Subsubclass, f"{row.ImageID}.jpg"
                )
            )
