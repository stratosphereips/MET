...
class AlexNet(Base):
    def __init__(
        ...
        self._set_parameter_requires_grad(self._alexnet, self._feature_extraction)
        self._alexnet.classifier[6].requires_grad_()

        if num_classes != 1000:
            in_features = self._alexnet.classifier[6].in_features
            self._alexnet.classifier[6] = nn.Linear(
...
class AlexNetSmall(nn.Module):
    def __init__(self, dims: Tuple[int, int, int], num_classes: int):
        ...
        self.features = nn.Sequential(
            nn.Conv2d(dims[0], 48, kernel_size=5, stride=1, padding=2),
